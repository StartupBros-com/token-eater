...
- **The review stage runs `/ce-code-review` as a real grok subagent fleet.** The recipe tells the service to review-and-fix its committed diff by following `/ce-code-review`'s method (apply safe, verified fixes, commit on the branch, never push), looping until no P0/P1 remain or `--rounds` is hit. Empirical finding from grok debug logs: grok's headless `Task` tool registers exactly four subagent types — `generalPurpose`, `code-reviewer`, `best-of-n-runner`, `cursor-guide` — and the skill's compound-engineering `ce-*` persona types (`ce-correctness-reviewer`, …) are **not** among them; dispatching one fails with `unknown variant ... expected one of [those four]`. So the recipe maps the review onto the types grok *does* have: one `code-reviewer` subagent over the whole diff **plus** one `generalPurpose` subagent per lens (correctness / tests / maintainability / security), run in parallel — a genuine multi-agent review on grok's own credits. (The actual Claude-native `ce-*` personas only run when the *work service is `claude`*.) Either way this is the service reviewing **its own** work, so still not a substitute for your independent review.
