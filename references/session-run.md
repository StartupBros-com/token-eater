...
**The reliable solution (and what the recipe now does): run the genuine `ce-*` reviewer *persona prompts* via the always-available `general-purpose` subagent.** The `ce-*` personas are just markdown files in the compound-engineering plugin (`~/.claude/plugins/marketplaces/*/plugins/compound-engineering/agents/ce-*.md`). `run-session.sh` discovers them at launch and the recipe tells the service to dispatch one `general-purpose` subagent per persona, each prompted to *read and fully adopt* its persona file, then review the diff. Verified: a `general-purpose` subagent reading `ce-correctness-reviewer.md` produces genuine correctness-reviewer output. This runs the real review personas every run, independent of grok's flaky Task registry. If the persona files aren't found, it falls back to generic `general-purpose` lens reviewers. Either way this is the service reviewing **its own** work, so still not a substitute for your independent review.
