...
want me to install it?" On yes, run `/plugin install compound-engineering@hov` (or
...
5. **Launch the session.** Call the engine; it does preflight (worktree off fresh `origin/main` + baseline gate), hands the service the recipe, then independently re-verifies the gate and ensures a draft PR:
   ...
   **You pass these flags, not the member.** `--trust-repo` is justified by the member's interactive
   ...
   into it (don't, by default). `--gate` is optional (auto-detected; see step 2). Add `--dry-run` to
...
| File | What it's for |
|------|---------------|
| `references/session-run.md` | **The main flow:** menu of token-heavy skills, how to pick a gate + target, and the `run-session.sh` contract |
